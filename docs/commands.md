...
```text
usage: public-diary-tools provision-github-app [-h]
...
domain-wide delegation for the service account, authorize only the Drive scopes needed, set
...
Create or reuse the app with `make python-tool ARGS="provision-github-app"`. The tool first checks whether local app
credentials are already installed on this repository with the required permissions. If setup is still needed, it offers
a numbered choice to create a new app or use an existing app. For an existing app, it checks permissions and repository
mapping, attempts to add this repository to selected-repository installations when needed, uploads
